...
class index {
	...
	loopItems() {
		...
		for (let index = 0; index < this.dataArray.length; index++) {
			...
			if ( 
				pairSectors.start[0] <= pairSectors.start[1] && pairSectors.end[1] <= pairSectors.end[0] ||
				pairSectors.start[1] <= pairSectors.start[0] && pairSectors.end[0] <= pairSectors.end[1]
			) {
				crossOverCount++;
			}
		}
